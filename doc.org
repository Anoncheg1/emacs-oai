...
1) ~oai-block--collect-chat-messages-at-point~ old: ~oai-restapi--collect-chat-messages-at-point~
...
test:
: (org-babel-expand-noweb-references (list "text" "<<ina()>>"))

*** TODO implementation
`oai-block-tags-get-content' call:
(oai-block-tags--apply-noweb) - that call `org-babel-expand-noweb-references', call oai--org-babel-get-src-block-info, but inly if reference is <<inf()>> with “()”
...
- `oai-block--pos-in-markdown-block-p' - any ``` block at separate lines.
