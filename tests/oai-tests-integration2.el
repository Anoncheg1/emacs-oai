      ...
      (let ((oai-restapi-con-endpoints (list :test "http://localhost:9239/v1/chat/completions"))
        ...
        (unwind-protect
            (progn
              ...
              (goto-char 1)
              (let* ((v (oai-restapi--collect-chat-messages (oai-block-get-content (oai-block-p))))
                     (p0 (seq-elt (oai-restapi--collect-chat-messages (oai-block-get-content (oai-block-p)))
                                  0))
                     (p1 (seq-elt (oai-restapi--collect-chat-messages (oai-block-get-content (oai-block-p)))
                                  1))
                     )
