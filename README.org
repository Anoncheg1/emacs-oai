...
- Highlighting: markdown block, Org tables, links. Commands to inspect raw data
...
3) Keep token in separate “auth-source” file with plain token text
To get more infomration use commnd:
: info auth-source
