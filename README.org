...
| File              | Purpose                                                            |
|-------------------+--------------------------------------------------------------------|
| oai.el            | minor Org oai-mode and ~C-c C-c~ activation of #+begin_ai          |
...
[*] According to the Free Software philosophy, in practice all remote LLMs are considered SaaSS (Service as a Software Substitute). Rather than communicating directly with users, these systems perform remote computation for every visitor. They not provide access to *data, model weights, code*, or licensing rights needed to reproduce the model.
* Installation
...
- org-src--contents-area = oai-block--area, oai-block--contents-area
* How this works?
