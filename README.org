...
For *highlighting* we use hack to inject our font-lock staff to ~font-lock-defaults~ variable initialized by ~org-set-font-lock-defaults~ function.
...
- ~oai-restapi-request-llm-retries~ - retrie request if it fails.
